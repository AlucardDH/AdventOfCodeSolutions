...
class Directory {
	...
	getWeight() {
		if(this.weight==-1) {
			this.weight = this.content.reduce((s,e)=>s+e.getWeight(),0);
		}
		return this.weight;
	}
	...
	getDirectory(path) {
		...
		return this.content.find(e=>e.name==path);
	}

	add(data) {
		var name = data[1];
		if(this.content.find(e=>e.name==name)) {
			return;
		}
		var element;
		if(data[0]=='dir') {
			element = new Directory(this,name);
		} else {
			element = new File(this,name,data[0]);
		}
...
input.forEach(e=>{
	...
	if(data[0]=='$') {
		// command
		if(data[1]=='cd') {
			currentDir = currentDir.getDirectory(data[2]);
			console.log(e,currentDir);
		} else if(data[1]='ls') {
			// skip
		}
	} else {
...
function part2() {
	var total = 70000000;
	var used = ROOT.getWeight();
	var remaining = total-used;
	var needed = 30000000;
	var toFree = needed-remaining;
	return allDirectories
		.sort((a,b)=>a.getWeight()-b.getWeight())
		.find(e=>e.getWeight()>=toFree)
		.getWeight();
}
